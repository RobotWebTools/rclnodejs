...
async function generateIDLFromAction(action) {
  ...
  await fse.writeFile(
    path.join(path.parse(action.filePath).dir, 'CMakeLists.txt'),
    cmakeFile,
    'utf8'
  );
  await fse.writeFile(
    path.join(path.parse(action.filePath).dir, 'package.xml'),
    packageFile,
    'utf8'
  );
...
function generateActionMessage(action) {
  return (
    `${action.interfaceName}ActionGoal action_goal\n` +
