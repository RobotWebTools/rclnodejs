...
class Node extends rclnodejs.ShadowNode {
  ...
  constructor(
    ...
    // TODO: fix this
    // if (!rclnodejs.isActiveContext(context)) {
    //   throw new Error('Invalid context. Must call rclnodejs.init(context) before using the context');
    // }

    let handle = rclnodejs.createNode(nodeName, namespace, context.handle);
    this.handle = handle;
    Object.defineProperty(this, 'handle', { configurable: false, writable: false }); // make read-only
    
    // TODO: consider renaming property context to _context
    this.context = context;
    this.context.addNode(this);

    this._init(nodeName, namespace, context, options);
    debug('Finish initializing node, name = %s and namespace = %s.', nodeName, namespace);
  }

  _init(name, namespace, context, options) {
    // context value set in constructor
