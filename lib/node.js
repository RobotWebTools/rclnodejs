...
class Node extends rclnodejs.ShadowNode {
    ...
    for (const subscription of subscriptionsReady) {
      ...
      if (subscription.isRaw) {
        if (subscription.wantsMessageInfo) {
          let taken = rclnodejs.rclTakeRawWithInfo(subscription.handle);
          if (taken) {
            subscription.processResponse(
              taken.buffer,
              new MessageInfo(taken.info)
            );
          }
        } else {
  ...
  createTimer(period, callback, optionsOrClock = null, clock = null) {
    ...
    let timer = new Timer(timerHandle, period, callback, timerClock);
  ...
  destroy() {
    ...
    // Timer handles are children of their clock handle rather than of the
    // node handle, so release them here instead of waiting for the clock to
    // be garbage collected.
    this._timers.forEach((timer) => timer.handle.release());
    this.handle.release();
    this._clock = null;
    this._timers = [];
    this._publishers = [];
