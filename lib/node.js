...
const Parameters = require('./parameter.js');
...
class NodeOptions {
   ...
   * @param {boolean} aBool - True indicates for a node to start it's parameter_service.
   *    Default value is true.
   */
  set startParameterServices(aBool) {
    this._startParameterServices = aBool;
  }
   ...
   * @param {Parameter[]} parameters - array of Parameters. Default is an empty array.
   */
  set parameterOverrides(parameters) {
    this._parameterOverrides = parameters;
  }
   ...
   * When true a node will declare parameters from all 
   * parameter-overrides.
   * @param {boolean} aBool - 
   */
  set automaticallyDeclareParametersFromOverrides(aBool) {
    this.automaticallyDeclareParametersFromOverrides = aBool;
  }
...
class Node {
  ...
  init(name, namespace, context, options) {
    ...
    this._parameterOverrides = this._getNativeParameterOverrides(context);

    // override cli parameterOverrides with those specified in options
    if (Array.isArray(options.parameterOverrides) && options.parameterOverrides.length > 0) {
      for (const parameter of options.parameterOverrides) {
        if (! parameter instanceof Parameter) {
          throw new TypeError('Parameter-override must be an instance of Parameter.');
        }
  ...
  declareParameter(parameter, descriptor, ignoreOveride = false) {
    const parameters = 
      this.declareParameters([parameter], descriptor ? [descriptor] : [], ignoreOveride);
  ...
  declareParameters(parameters, descriptors=[], ignoreOverrides = false) {
    ...
    if (!Array.isArray(descriptors)) {
      throw new TypeError('Invalid parameters: expected array of ParameterDescriptor');
    }

    if (descriptors.length > 0 && (parameters.length !== descriptors.length)) {
      throw new TypeError('Each parameter must have a cooresponding ParameterDescriptor');
    }
    ...
    for (let i=0; i < parameters.length; i++) {
      ...
      let descriptor =  descriptors.length > 0 ? 
        descriptors[i] : 
    ...
    if (declaredParameterCollisions.length > 0) {
      errorMsg = declaredParameterCollisions.length == 1 ?
  ...
  undeclareParameter(name) {
    ...
    if (descriptor.readOnly) {
      throw new Error(`${name} parameter is read-only and can not be undeclared`);
    }
  ...
  getParameters(names = []) {
    ...
    if (names.length == 0) { // get all parameters
  ...
  _setParametersAtomically(parameters=[], declareParameterMode=false) {
    ...
    const parameterEvent = new (loader.loadInterface(PARAMETER_EVENT_MSG_TYPE));
    
	  const secondsAndNanos = (new  Clock.ROSClock()).now().secondsAndNanoseconds;
    parameterEvent.stamp = 
      { sec: secondsAndNanos.seconds, nanosec: secondsAndNanos.nanoseconds };

    parameterEvent.node = this.namespace() === '/' ?
      this.namespace() + this.name() :
      this.namespace() + '/' + this.name();
    
    if (newParameters.length > 0) {
      // eslint-disable-next-line camelcase
      parameterEvent.new_parameters = 
        newParameters.map(parameter => parameter.toParameterMessage());
    }
    if (changedParameters.length > 0) {
      // eslint-disable-next-line camelcase
      parameterEvent.changed_parameters = 
        changedParameters.map(parameter => parameter.toParameterMessage());
    }
    if (deletedParameters.length > 0) {
      // eslint-disable-next-line camelcase
      parameterEvent.deleted_parameters = 
  ...
  _getNativeParameterOverrides(context) {
    ...
    //     value: object
    // }
    const cliParamOverrideData = rclnodejs.getParameterOverrides(context.handle());
    ...
    if (cliParamOverrideData) {
      for (let nodeParamData of cliParamOverrideData) {
        ...
        for (let paramData of nodeParamData.parameters) {
          const paramOverride = new Parameters.Parameter(paramData.name, paramData.type, paramData.value);
