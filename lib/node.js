...
const { ParameterType, Parameter, ParameterDescriptor } = require('./parameter.js');
...
class Node {
    ...
    actionClientsReady.forEach((actionClient) => {
      ...
      if (properties.isFeedbackReady) {
        this._runWithMessageType(
          ...
          (message, deserialize) => {
            let success = rclnodejs.actionTakeFeedback(actionClient.handle, message);
            if (success) {
    ...
    actionServersReady.forEach((actionServer) => {
      ...
      if (properties.isGoalRequestReady) {
        this._runWithMessageType(
          ...
          (message, deserialize) => {
            const result = rclnodejs.actionTakeGoalRequest(actionServer.handle, message);
            if (result) {
      ...
      if (properties.isCancelRequestReady) {
        this._runWithMessageType(
          ...
          (message, deserialize) => {
            const result = rclnodejs.actionTakeCancelRequest(actionServer.handle, message);
            if (result) {
      ...
      if (properties.isResultRequestReady) {
        this._runWithMessageType(
          ...
          (message, deserialize) => {
            const result = rclnodejs.actionTakeResultRequest(actionServer.handle, message);
            if (result) {
  ...
  createTimer(period, callback, clock = null) {
    ...
    let timerHandle = rclnodejs.createTimer(timerClock.handle, this.context.handle, period);
  ...
  declareParameters(parameters, descriptors = [], ignoreOverrides = false) {
    ...
    for (let i = 0; i < parameters.length; i++) {
      ...
      let descriptor =
        descriptors.length > 0 ? descriptors[i] : ParameterDescriptor.fromParameter(parameter);
  ...
  setParameters(parameters = []) {
    return parameters.map((parameter) => this.setParametersAtomically([parameter]));
  }
