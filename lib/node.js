...
class Node {
  ...
  execute() {
    ...
    this._subscriptions.forEach((subscription) => {
      let rosMsg = rclnodejs.rclTake(subscription.handle, subscription.rosMsg);
      if (rosMsg) {
        subscription.processResponse(rosMsg);
      }
    });
  }

  handle() {
    console.log('get handle = ' + this._handle);
    return this._handle;
  }

  setHandle(handle) {
    this._handle = handle;
    console.log('set handle = ' + this._handle);
  }
