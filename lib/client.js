...
class Client extends Entity {
  ...
  sendRequest(request, callback) {
    ...
    this._pending_requests.set(sequenceNumber, callback);
  }

  processResponse(sequence_number, response) {
    if (this._pending_requests.has(sequence_number)) {
      debug(`Client has received ${this._serviceName} response from service.`);
      let callback = this._pending_requests.get(sequence_number);
      this._pending_requests.delete(sequence_number);
      callback(response.toPlainObject(this.typedArrayEnabled));
    } else {
      error(`Client has received an unexpected ${this._serviceName}
             response with sequence number ${sequence_number}.`);
    }
  }
