...
class Response {
  ...
  get template() {
    return MessageTranslator.toPlainObject(new this._service._typeClass.Response());
  }
   ...
   */
  send(response) {
    const rawROSResponse = this._service.translator.toROSResponse(response).serialize();
