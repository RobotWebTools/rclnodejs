...
/**
 * Resolve a capability payload (message, service Request/Response, or
 * action Goal/Feedback/Result) to a JSON Schema, without registering
 ...
 * (`CapabilityRegistry.list()` has call, publish, subscribe, and action
 * maps from capability names to ROS type names).
 *
 ...
 *
 * @param {{call: object, publish: object, subscribe: object, action?: object}} capabilities
...
function buildOpenApiDocument(capabilities, options = {}) {
  ...
  for (const [name, typeName] of Object.entries(capabilities.action || {})) {
    const route = `${basePath}/action${name}`;
    paths[route] = {
      post: {
        summary: `Send a ROS 2 action goal to ${name}`,
        operationId: `action_${sanitizeName(name)}`,
        'x-ros-capability': { kind: 'action', name, type: typeName },
        description:
          'POST a JSON goal to receive accepted and feedback events, then a ' +
          'terminal result or error event. Result data contains status and payload. ' +
          'Use fetch() or curl -N; EventSource does not support POST. ' +
          'Use WebSocket for cancellation; disconnecting does not cancel the goal.',
        requestBody: {
          required: true,
          content: {
            'application/json': {
              schema: topLevelSchema(typeName, 'Goal', components),
            },
          },
        },
        responses: {
          200: {
            description: `Server-Sent Events stream for ${typeName}`,
            content: {
              'text/event-stream': {
                schema: {
                  description:
                    'JSON data for each SSE event; titles name the event.',
                  anyOf: [
                    {
                      title: 'accepted',
                      type: 'object',
                    ...
                    {
                      title: 'feedback',
                      ...topLevelSchema(typeName, 'Feedback', components),
                    },
                    {
                      title: 'result',
                      type: 'object',
                      properties: {
                        status: {
                          type: 'string',
                          enum: ['succeeded', 'canceled', 'aborted', 'unknown'],
                        },
                        payload: topLevelSchema(typeName, 'Result', components),
                      },
          ...
          404: notExposedResponse(),
          409: {
            description: 'Action goal rejected before streaming',
            content: {
              'application/json': {
                schema: {
                  type: 'object',
                  properties: {
                    ...
                    error: { type: 'string' },
                    code: { type: 'string', const: 'goal_rejected' },
                  },
                },
              },
            },
