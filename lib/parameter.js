...
class Parameter {
  ...
  toParameterValueMessage() {
    ...
    case ParameterType.PARAMETER_BOOL:
      ...
      break;
    case ParameterType.PARAMETER_BOOL_ARRAY:
      msg.bool_array_value = this.value.map(val => val ? 'true' : 'false');
      break;
    case ParameterType.PARAMETER_BYTE_ARRAY:
...
class ParameterDescriptor {
   ...
   */
  constructor(name, type = ParameterType.PARAMETER_NOT_SET, 
    description='no description', readOnly=false, range=null) {
  ...
  set range(range) {
    ...
    if (! this.range.isValidType(this.type)) {
  ...
  validate() {
    ...
    if (this.hasRange() && 
       !this.range.isValidType(this.type)) {
  ...
  validateParameter(parameter) {
    ...
    if (this.hasRange() && !this.getRange().inRange(parameter.value)) {
      throw new Error('Parameter fails descriptor range');
    }
  }
  ...
  toMessage() {
    ...
    if ((this._type == ParameterType.PARAMETER_INTEGER || 
    ...
    } else if ((this._type == ParameterType.PARAMETER_DOUBLE ||
...
class Range {
  ...
  inRange(value) {
    if (Array.isArray(value)) {
      const valArr = value;
      return valArr.reduce((inRange, val) => inRange && this.inRange(val), true);
...
class FloatingPointRange extends Range {
  ...
  inRange(value) {
    ...
    if (IsClose.isClose(value, min, PARAMETER_REL_TOL) || 
...
function parameterTypeFromValue(value) {
  ...
  if (Array.isArray(value)) {
    if (value.length > 0) {
      ...
      switch (elementType) {
      case ParameterType.PARAMETER_BOOL:   return ParameterType.PARAMETER_BOOL_ARRAY;
      case ParameterType.PARAMETER_DOUBLE: return ParameterType.PARAMETER_DOUBLE_ARRAY;
      case ParameterType.PARAMETER_STRING: return ParameterType.PARAMETER_STRING_ARRAY;
      }
    }
...
function validType(parameterType) {
  ...
  let result = typeof parameterType === 'number' && 
      (ParameterType.PARAMETER_NOT_SET <= parameterType <= ParameterType.PARAMETER_STRING_ARRAY);
...
function validValue(value, type) {
  ...
  case ParameterType.PARAMETER_DOUBLE_ARRAY:
    ...
    if (result) {
      ...
      result = 
        values.reduce((compatible, val) => 
          compatible && validValue(val, arrayElementType), 
        true);
    }
    break;
