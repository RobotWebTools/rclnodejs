...
class HttpRequestConnection extends Connection {
   ...
   * @param {*} payload
   */
  constructor(request, response, kind, capability, payload) {
    super();
    ...
    // NOTE: we deliberately do *not* wire `request.on('close')` /
    // `response.on('close')` to `_emitCloseOnce()` here. Doing so would
...
/** Shared SSE framing, heartbeats, and response lifecycle. */
class HttpSseConnection extends Connection {
  ...
  constructor(request, response, options = {}) {
    ...
    // Use response: request.close marks POST body completion, not client disconnect.
 ...
 */
class HttpSubscriptionConnection extends HttpSseConnection {
  /**
   * @param {import('http').IncomingMessage} request
   * @param {import('http').ServerResponse} response
...
/** Streams one action over SSE; disconnecting does not cancel the goal. */
class HttpActionConnection extends HttpSseConnection {
  /**
   * @param {import('http').IncomingMessage} request
   * @param {import('http').ServerResponse} response
   * @param {string} capability ROS action name.
   * @param {*} payload Parsed goal.
   * @param {object} [options]
   * @param {number} [options.keepAliveMs=15000] Heartbeat interval.
   */
  constructor(request, response, capability, payload, options = {}) {
    super(request, response, options);
    this._capability = capability;
    this._payload = payload;
  ...
  send(frame) {
    ...
    // Failures before streaming use HTTP errors, not SSE events.
    const code = frame.code || 'internal_error';
    const status = _STATUS_BY_CODE[code] || 500;
 ...
 *
 * Action goals use a JSON POST with an SSE response:
 *
 *     POST /capability/action/<name>     (text/event-stream response)
 *
 * Streams `accepted`, `feedback`, then one terminal `result` or `error`.
 * Use WebSocket for cancellation; an HTTP disconnect does not cancel the goal.
 *
...
class HttpTransport extends TransportAdapter {
   ...
   *   transport.
   * @param {(request: import('http').IncomingMessage) => boolean} [options.verifyRequest]
  ...
  _route(request, response) {
    ...
    this._applyCors(request, response);

    let pathname;
    try {
      pathname = new URL(request.url || '/', 'http://localhost').pathname;
    } catch {
      return _writeJson(response, 400, {
        ok: false,
    ...
    if (!pathname.startsWith(this.basePath + '/')) {
      return _writeJson(response, 404, {
        ok: false,
    ...
    if (this.verifyRequest) {
      let allowed;
      try {
        allowed = this.verifyRequest(request);
      } catch (e) {
        debug('verifyRequest threw: %s', (e && e.stack) || e);
        return _writeJson(response, 500, {
          ok: false,
      ...
      if (allowed === false) {
        return _writeJson(response, 401, {
          ok: false,
          error: 'unauthorized',
          code: 'unauthorized',
        });
    ...
    if (kind === 'subscribe') {
      ...
      if (request.method !== 'GET') {
        ...
        return _writeJson(response, 405, {
          ok: false,
          error: `method not allowed: ${request.method} (use GET for SSE subscribe)`,
          code: 'method_not_allowed',
        });
      }
      const conn = new HttpSubscriptionConnection(request, response, name, {
    ...
    _readJsonBody(request, _MAX_BODY_BYTES, (err, payload) => {
      if (err) {
      ...
      const conn =
        kind === 'action'
          ? new HttpActionConnection(request, response, name, payload, {
              keepAliveMs: this.sseKeepAliveMs,
            })
          : new HttpRequestConnection(request, response, kind, name, payload);
      try {
        this._onConnection(conn);
        conn.begin();
      } catch (e) {
  ...
  _applyCors(request, response) {
    ...
    if (requested) {
      response.appendHeader('vary', 'Access-Control-Request-Headers');
...
function _writeJson(response, status, body) {
...
function _readJsonBody(request, maxBytes, cb) {
  // Guard against double-callback: the body may exceed maxBytes (we call
  // cb(err) and request.destroy()), and the destroy itself can synchronously
  ...
  const ctype = (request.headers['content-type'] || '').toLowerCase();
