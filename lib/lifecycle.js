 ...
 * The Lifecycle module depends upon the type-support system
 * to be properly configured, i.e., generated messages, before
 * use. Thus rclnodejs.init() should be called before call
 * this function.
 * 
...
function initialize() {
  ...
  CallbackReturnCode.SUCCESS = TransitionInterface.TRANSITION_CALLBACK_SUCCESS;
  CallbackReturnCode.FAILURE = TransitionInterface.TRANSITION_CALLBACK_FAILURE;
  CallbackReturnCode.ERROR = TransitionInterface.TRANSITION_CALLBACK_ERROR;
...
class State {
   ...
   * @return {SerializedState} The object.
   */
  asMessage() {
...
class TransitionDescription {
  ...
  /**
   * Create an object representation of the transitionDescriptons properties.
   * 
  ...
  asMessage() {
    {
      return ({
        transition: this.transition.asMessage(),

        // eslint-disable-next-line camelcase
        start_state: this.startState.asMessage(),

        // eslint-disable-next-line camelcase
        goal_state: this.goalState.asMessage()
      });
    }
  }
}
...
class CallbackReturnValue {
   ...
   */
  hasErrorMsg() {
    return this.isSuccess() ? false : !!this._errorMsg;
  }
}
...
class LifecycleNode extends rclnodejs.ShadowNode {

  init(name, namespace, context, options) {
    super.init(name, namespace, context, options);

    // initialize the module if needed.
    Lifecycle.initialize();

    // initialize native handle to rcl_lifecycle_state_machine
    this._stateMachineHandle = rclnodejs.createLifecycleStateMachine(this.handle);

    // initialize Map<transitionId,TransitionCallback>
    this._callbackMap = new Map();

    // Setup and register the 4 native rcl lifecycle services attached 
    // to _stateMachineHandle.
    let srvHandleObj =
      rclnodejs.getLifecycleSrvNameAndHandle(
        this.handle,
  ...
  get currentState() {
    let currentStateObj =
      rclnodejs.getCurrentLifecycleState(this._stateMachineHandle);
    return this._newState(currentStateObj);
  }

  // TODO: Wayne - check with rclcpp team for clarity on their impl that 
  // returns all registered states. Based on the name of this method
  // it seems it should be consistent with the rclcpp on_get_avalable_transitions()
  // in that it should return the available primary goal_states from the current state rather
  // than all states in the state-machine.
  /**
  ...
  get availableStates() {
    ...
    let states = stateObjs.map((stateObj) => new State(stateObj.id, stateObj.label));
    return states;
  }
   ...
   */
  get transitions() {
    let transitionObjs = rclnodejs.getLifecycleTransitions(this._stateMachineHandle);
    let transitions = transitionObjs.map(
      (transitionDescObj) => {
        let transition = new Transition(transitionDescObj.transition.id, transitionDescObj.transition.label);
        let startState = new State(transitionDescObj.start_state.id, transitionDescObj.start_state.label);
        let goalState = new State(transitionDescObj.goal_state.id, transitionDescObj.goal_state.label);
  ...
  get availableTransitions() {
    let transitionObjs = rclnodejs.getAvailableLifecycleTransitions(this._stateMachineHandle);
    let transitions = transitionObjs.map(
      (transitionDescObj) => {
        let transition = new Transition(transitionDescObj.transition.id, transitionDescObj.transition.label);
        let startState = new State(transitionDescObj.start_state.id, transitionDescObj.start_state.label);
  ...
  registerOnConfigure(cb) {
    this._callbackMap.set(
      StateInterface.TRANSITION_STATE_CONFIGURING,
      cb);
  }
  ...
  registerOnActivate(cb) {
    this._callbackMap.set(
      StateInterface.TRANSITION_STATE_ACTIVATING,
      cb);
  }
  ...
  registerOnDeactivate(cb) {
    this._callbackMap.set(
      StateInterface.TRANSITION_STATE_DEACTIVATING,
      cb);
  }
  ...
  registerOnCleanup(cb) {
    this._callbackMap.set(
      StateInterface.TRANSITION_STATE_CLEANINGUP,
      cb);
  }
  ...
  registerOnShutdown(cb) {
    this._callbackMap.set(
      StateInterface.TRANSITION_STATE_SHUTTINGDOWN,
      cb);
  }
  ...
  registerOnError(cb) {
    this._callbackMap.set(
      StateInterface.TRANSITION_STATE_ERRORPROCESSING,
      cb);
  }
  ...
  configure(callbackReturnValue) {
    return this._changeState(TransitionInterface.TRANSITION_CONFIGURE, callbackReturnValue);
  }
  ...
  activate(callbackReturnValue) {
    return this._changeState(TransitionInterface.TRANSITION_ACTIVATE, callbackReturnValue);
  }
  ...
  deactivate(callbackReturnValue) {
    return this._changeState(TransitionInterface.TRANSITION_DEACTIVATE, callbackReturnValue);
  }
  ...
  cleanup(callbackReturnValue) {
    return this._changeState(TransitionInterface.TRANSITION_CLEANUP, callbackReturnValue);
  }

  /**
   * Initiate a transition from either the UNCONFIGURED state, INACTIVE state or ACTIVE state
   * to the FINALIZED state. If an onConfigure callback has been registered it will be invoked.
   * 
   * @param {CallbackReturnValue?} callbackReturnValue - value holder for the CallbackReturnCode returned from the callback.
  ...
  shutdown(callbackReturnValue) {
    ...
    if (state.id === StateInterface.PRIMARY_STATE_UNCONFIGURED) {
      return this._changeState(TransitionInterface.TRANSITION_UNCONFIGURED_SHUTDOWN, callbackReturnValue);
    }
    if (state.id === StateInterface.PRIMARY_STATE_INACTIVE) {
      return this._changeState(TransitionInterface.TRANSITION_INACTIVE_SHUTDOWN, callbackReturnValue);
    }
    if (state.id === StateInterface.PRIMARY_STATE_ACTIVE) {
      return this._changeState(TransitionInterface.TRANSITION_ACTIVE_SHUTDOWN, callbackReturnValue);
    }

    // todo: what to do if get to this line
  }
  ...
  _onGetState(request, response) {
    ...
    // eslint-disable-next-line camelcase
    result.current_state = this.currentState.asMessage();
  ...
  _onGetAvailableStates(request, response) {
    ...
    let result = response.template;
    // eslint-disable-next-line camelcase
    result.available_states = states.map((state) => state.asMessage());
  ...
  _onGetAvailableTransitions(request, response) {
    let transitionDescs = this.availableTransitions;
    let transitionDescObjs = transitionDescs.map((transitionDesc) => transitionDesc.asMessage());
    
    let result = response.template;
    // eslint-disable-next-line camelcase
    result.available_transitions = transitionDescObjs;
  ...
  _onChangeState(request, response) {
    ...
    if (request.transition.label) {
      let transitionObj =
        rclnodejs.getLifecycleTransitionByLabel(this._stateMachineHandle, request.transition.label);
      if (transitionObj.id) {
  ...
  _changeState(transitionIdOrLabel, callbackReturnValue) {
    ...
    let newStateObj = typeof transitionIdOrLabel === 'number' ?
      rclnodejs.triggerLifecycleTransitionById(this._stateMachineHandle, transitionIdOrLabel) :
      rclnodejs.triggerLifecycleTransitionByLabel(this._stateMachineHandle, transitionIdOrLabel);

    if (!newStateObj) {
      throw new Error(`No transition available from state ${transitionIdOrLabel}.`);
    }

    let newState = this._newState(newStateObj);
    ...
    newState = this._newState(
      rclnodejs.triggerLifecycleTransitionByLabel(this._stateMachineHandle, transitioningLabel)
    );
