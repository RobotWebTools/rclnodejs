...
NAN_METHOD(ActionAcceptNewGoal) {
  ...
  auto js_obj = RclHandle::NewInstance(goal_handle);

  info.GetReturnValue().Set(js_obj);
}

NAN_METHOD(ActionDestroyServerGoalHandle) {
  RclHandle* goal_handle_handle = RclHandle::Unwrap<RclHandle>(
      Nan::To<v8::Object>(info[0]).ToLocalChecked());
  rcl_action_goal_handle_t* goal_handle =
      reinterpret_cast<rcl_action_goal_handle_t*>(goal_handle_handle->ptr());

  THROW_ERROR_IF_NOT_EQUAL(rcl_action_goal_handle_fini(goal_handle),
                           RCL_RET_OK,
                           rcl_get_error_string().str);
...
BindingMethod action_binding_methods[] = {
    ...
    {"actionDestroyServerGoalHandle", ActionDestroyServerGoalHandle},
