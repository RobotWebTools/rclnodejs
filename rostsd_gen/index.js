...
function saveMsgWrappersAsTSD(pkgs) {
  const wrappersDir = `${__dirname}/../generated`;
  for (const pkgInfo of pkgs) {
    const pkgDir = `${wrappersDir}/${pkgInfo.name}`;
    fs.mkdirSync(pkgDir, { recursive: true });
    for (const msgInfo of pkgInfo.messages) {
      const typeClass = msgInfo.typeClass;
      const mangledName = `${typeClass.package}__${typeClass.type}__${typeClass.name}`;

      const fd = fs.openSync(`${pkgDir}/${mangledName}.d.ts`, 'w');
      ...
      fs.writeSync(fd, `declare class ${typeClass.name}Wrapper {\n`);
      ...
      // fields
      saveMsgFieldsAsTSD(msgInfo, fd, 2, ';', 'rclnodejs.');
      ...
      fs.writeSync(fd, `declare namespace ${typeClass.name}Wrapper { }\n\n`);
